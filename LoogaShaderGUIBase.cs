...
namespace LoogaSoft.Lighting.Editor
{
    public abstract class LoogaShaderGUIBase : ShaderGUI
    {
        ...
        {
            MaterialProperty tf2SpecularMask = FindProperty("_TF2SpecularMask", properties, false);
            MaterialProperty tf2SpecularStrength = FindProperty("_TF2SpecularStrength", properties, false);
            MaterialProperty tf2PhongExponent = FindProperty("_TF2PhongExponent", properties, false);
            MaterialProperty tf2RimMask = FindProperty("_TF2RimMask", properties, false);
            MaterialProperty tf2RimStrength = FindProperty("_TF2RimStrength", properties, false);
            MaterialProperty tf2RimExponent = FindProperty("_TF2RimExponent", properties, false);
            ...
            if (tf2SpecularMask == null)
                return;

            Section("Lighting Model Inputs", prefKey, false, () =>
            {
                GUILayout.Label("Team Fortress 2", EditorStyles.boldLabel);
                materialEditor.TexturePropertySingleLine(new GUIContent("Phong Mask"), tf2SpecularMask, tf2SpecularStrength);
                materialEditor.ShaderProperty(tf2PhongExponent, "Phong Exponent");
                materialEditor.TexturePropertySingleLine(new GUIContent("Rim Mask"), tf2RimMask, tf2RimStrength);
                materialEditor.ShaderProperty(tf2RimExponent, "Rim Exponent");
