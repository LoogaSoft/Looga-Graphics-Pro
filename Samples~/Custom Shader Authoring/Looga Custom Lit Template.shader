Shader "LoogaSoft/Samples/Custom Lit Template"
{
    // Working authoring template for custom opaque/cutout materials. Keep the
    // Looga pass tags, stencil classification, packing helpers, and sidecars;
    // replace the properties and sampling code with the material's own inputs.
    Properties
    {
        ...
        [NoScaleOffset] _TF2SpecularMask ("TF2 Phong Mask", 2D) = "white" {}
        _TF2SpecularStrength ("TF2 Phong Strength", Range(0, 2)) = 1.0
        _TF2PhongExponent ("TF2 Phong Exponent", Range(1, 256)) = 32.0
        [NoScaleOffset] _TF2RimMask ("TF2 Rim Mask", 2D) = "white" {}
        _TF2RimStrength ("TF2 Rim Strength", Range(0, 4)) = 0.35
        _TF2RimExponent ("TF2 Rim Exponent", Range(1, 16)) = 4.0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 FragForward(VaryingsForward input, bool isFrontFace : SV_IsFrontFace) : SV_Target
            {
                ...
                bool mainLightMatchesLayers = LoogaLightMatchesRenderingLayer(mainLight, meshRenderingLayers);
                float3 mainRadiance = mainLightMatchesLayers ? mainLight.color * mainLight.shadowAttenuation * mainLight.distanceAttenuation : 0.0;
                ...
                float3 color = EvaluateLoogaAdditionalLight(mainLight, meshRenderingLayers, diffuseColor, directF0, perceptualRoughness, normalWS, 1.0, viewDirWS, NoV, modelParameters, false, false, 0.0, 0.0);
                    ...
                    half thickness = SAMPLE_TEXTURE2D(_ThicknessMap, sampler_BaseMap, input.uv).r;
                    half transmissionMask = (1.0 - thickness) * _TransmissionStrength;
                    color += EvaluateTransmission(_SubsurfaceColor.rgb, _ScatterWidth, _AmbientScatterStrength, _TransmissionShadowSoftness, mainLight.direction, viewDirWS, normalWS, mainRadiance, mainLight.shadowAttenuation, transmissionMask);
                #endif
        ...
        Pass
        {
            ...
            ProfileOutput FragProfile(VaryingsProfile input)
            {
                ...
                output.profileExtra = half4(_AmbientScatterStrength / 5.0, _TransmissionShadowSoftness, 0, 1);
                return output;
            }
            ENDHLSL
        }
